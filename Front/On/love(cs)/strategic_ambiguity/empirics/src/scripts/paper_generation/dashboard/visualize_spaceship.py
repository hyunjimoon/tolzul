...
def generate_dashboard():
    ...
    jeong_agents = [
        {"id": "j1", "name": "8JID", "role": "Intro", "icon": "🐢", "onclick": "launchApp('Claude')"},
        {"id": "j2", "name": "9JT", "role": "Theory", "icon": "🐢", "onclick": "launchApp('Claude')"},
        {"id": "j3", "name": "10JE", "role": "Empirics", "icon": "🐢", "onclick": "launchApp('Claude')"}
    ]

    # 2. Na (Orange) - Structure (3:00) -> 6, 5, 4
    na_agents = [
        {"id": "n1", "name": "6GID", "role": "Struct", "icon": "🐅", "onclick": "launchApp('Claude')"},
        {"id": "n2", "name": "5GT", "role": "Logic", "icon": "🏗️", "onclick": "launchApp('Claude')"},
        {"id": "n3", "name": "4GE", "role": "Code", "icon": "💻", "onclick": "launchApp('Claude')"}
    ]

    # 3. O (Blue) - Origin (9:00) -> 11, 12, 13
    o_agents = [
        {"id": "o1", "name": "11OU", "role": "MgmtSci", "icon": "💾", "onclick": ""},
        {"id": "o2", "name": "12OC", "role": "Strategy", "icon": "📚", "onclick": ""},
        {"id": "o3", "name": "13ON", "role": "OpsMgmt", "icon": "📜", "onclick": ""}
    ]

    # 4. Kim (Red) - Evaluation (12:00) -> 1, 2, 3
    kim_agents = [
        {"id": "k1", "name": "1KU", "role": "Audit U", "icon": "🛡️", "onclick": "launchApp('Antigravity')"},
        {"id": "k2", "name": "2KC", "role": "Audit C", "icon": "⚖️", "onclick": "launchApp('Antigravity')"},
        {"id": "k3", "name": "3KN", "role": "Audit N", "icon": "💪", "onclick": "launchApp('Antigravity')"}
    ]

    # 5. Moon (Center) - Commander -> 7M
    moon_agent = {"id": "m1", "name": "7M", "role": "Commander", "icon": "🌙", "onclick": ""}
        ...
        <div class="server-status">
            <div class="monitor-map" style="display: flex; flex-direction: column; align-items: flex-end; margin-right: 15px; opacity: 0.8;" title="Physical Monitor Layout: Left(Vertical), Center(Main), Bottom(Small)">
                ...
                <div style="display: flex; gap: 4px; align-items: flex-end;">
                    <div style="width: 10px; height: 18px; background: #333; border: 1px solid #555; border-radius: 2px;" title="Left Vertical Monitor"></div>
                    <div style="display: flex; flex-direction: column; gap: 2px; align-items: center;">
                        <div style="width: 26px; height: 14px; background: var(--jeong-green); border: 1px solid var(--jeong-green); border-radius: 2px; box-shadow: 0 0 5px var(--jeong-green);" title="Center Main Monitor"></div>
                        <div style="width: 18px; height: 8px; background: #333; border: 1px solid #555; border-radius: 2px;" title="Bottom Small Monitor"></div>
                    </div>
                </div>
            </div>
