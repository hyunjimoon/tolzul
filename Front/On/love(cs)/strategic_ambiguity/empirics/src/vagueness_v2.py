...
class HybridVaguenessScorerV2:
    """
    Hybrid Vagueness Scorer: Combines V2 linguistic features with concrete feature counts.

    This scorer provides superior discriminatory power by combining:
    - 50% V2 linguistic vagueness (categorical terms + concreteness deficit)
    - 50% inverse concrete feature count (numbers, dates, units, metrics)

    Parameters
    ----------
    use_idf : bool, default=True
        Weight abstract terms by inverse document frequency in V2 component
    random_state : int, default=42
        Random seed for reproducibility
    """

    def __init__(self, use_idf: bool = True, random_state: int = 42):
        self.use_idf = use_idf
        self.random_state = random_state
        self.v2_scorer = StrategicVaguenessScorerV2(use_idf=use_idf, random_state=random_state)
        self._fitted = False

    def _count_concrete_features(self, text: str) -> float:
        """Count concrete features in text (numbers, dates, units, metrics)."""
        if not isinstance(text, str):
            return 0
    ...
    def fit(self, texts: List[str], y: Optional[pd.Series] = None, group_cols: Optional[pd.DataFrame] = None):
        """
        Fit the hybrid scorer on a corpus.
        ...
        y : pd.Series, optional
            Ignored (for sklearn compatibility)
        group_cols : pd.DataFrame, optional
            Group columns (ignored in this version)
        ...
        """
        # Fit V2 component
        self.v2_scorer.fit(texts, y, group_cols)

        # Compute concrete feature statistics for normalization
        concrete_counts = pd.Series([self._count_concrete_features(text) for text in texts])
        self.max_concrete = concrete_counts.quantile(0.95)  # Cap outliers

        self._fitted = True
        return self

    def transform(self, texts: List[str], group_cols: Optional[pd.DataFrame] = None) -> np.ndarray:
        """
        Transform texts into hybrid vagueness scores.
        ...
        -------
        np.ndarray
            Shape (n_samples, 5): [S_cat, S_concdef, V_raw, V_pct, V_minmax]
            V_minmax column (index 4) contains the final hybrid score
        """
        if not self._fitted:
            raise RuntimeError("Must call fit() before transform()")

        # Get V2 scores
        v2_df = self.v2_scorer.transform(texts, group_cols)
        v2_scores = v2_df['V_minmax'].values

        # Get concrete feature scores (inverse)
        concrete_counts = pd.Series([self._count_concrete_features(text) for text in texts])
        concrete_scores = 100 - (concrete_counts.clip(0, self.max_concrete) / self.max_concrete * 100)

        # Combine 50/50
        hybrid_scores = 0.5 * v2_scores + 0.5 * concrete_scores.values

        # Return in same format as V2 scorer (for compatibility)
        # Copy V2's S_cat, S_concdef, V_raw, V_pct but replace V_minmax with hybrid
        result = v2_df.values.copy()
        result[:, 4] = hybrid_scores  # Replace V_minmax column

        return result

    def fit_transform(self, texts: List[str], y: Optional[pd.Series] = None, group_cols: Optional[pd.DataFrame] = None) -> np.ndarray:
        """
        Fit and transform in one step.
