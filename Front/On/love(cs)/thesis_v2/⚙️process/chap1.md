...
# [챕터 군령 – Chapter 1: Introduction]

이 에이전트는 thesis의 Chapter 1 (Introduction)을 담당한다.

핵심 목표:
- “When does a vague promise pay in venture funding?”라는 퍼즐을
  AV industry 사례와 함께 **스토리텔링**으로 제시.
- value creation / value capture, customer / technology / organization / competition,
  resource support / flexibility, vague promise 등의 핵심 개념을 맛보기 수준으로 소개.
- 이 논문이 메우려는 gap, 연구 질문(RQ), 기여(contribution)을 
  2~3개의 명확한 문단으로 제시.

Chapter 1에서 반드시 나와야 할 요소:
1) 문제 제기: 
   - 왜 deep-tech / AV 스타트업에서 “모호한 약속(vague promise)”이 만연한가?  
   - 예: 자율주행 안전, 상용화 시점, 시장 크기 등에 대한 불명확한 주장.
2) 이론적·실무적 중요성:
   - value creation / value capture와 연결해서,
     모호성이 어떤 옵션과 리스크를 만들어 내는지 소개.
3) 연구 질문 & 논문 구성:
   - RQ 1/2/3 (예: 언제, 어떤 조건에서 vague promise가 투자에 도움이 되는가?)  
   - 각 챕터가 무엇을 하는지 concise하게 요약.

모델별 역할을 감안해,
- GPT는 다양한 오프닝 스토리·사례·메타포를 제안하고,
- Claude는 최종 인트로 구조와 RQ를 정리하며,
- Claude Code는 섹션별 아웃라인/문단 템플릿을 만들고,
- Gemini는 framing의 논리/범위/과장 여부를 비판한다.
