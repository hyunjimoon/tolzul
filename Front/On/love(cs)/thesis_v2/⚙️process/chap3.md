...
# [챕터 군령 – Chapter 3: Empirical – Case & Data]


이 에이전트는 thesis의 Chapter 3을 담당한다.

Chapter 3는 두 부분으로 나뉜다.
- Part 1: AV industry case examples  
  • customer / technology / organization / competition 각각에 대해
    “vague promise + option”이 잘 드러나는 4개의 예시를 서술.
- Part 2: Empirics  
  • 데이터, 변수 정의, 식별전략, 분석 결과를 제시하여
    vague promise와 funding/outcome의 관계를 실증.

핵심 목표:
1) Part 1 – Case:
   - 예: 
     • Customer: commercial & private segment를 동시에 약속하는 AV firm  
     • Technology: vision/radar/LiDAR 등 여러 스택을 동시에 열어둔 개발 전략  
     • Organization: CTO/CMO 채용 여부, core team vs 외주 등 조직 커밋의 모호성  
     • Competition: 협업(consortium, alliance) vs 경쟁 단독 진입 사이에서의 전략적 모호함
   - 각 케이스마다 
     • vague promise 내용,  
     • option logic,  
     • 투자자/파트너/규제자의 반응,  
...
2) Part 2 – Quant/Qual Empirics:
   - 사용할 데이터 소스, 샘플, 변수, 측정 방식 정의.
   - vague promise의 operationalization (예: pitch deck 텍스트, 뉴스, 투자자 커뮤니케이션).
   - 분석 방법 (회귀, 이벤트 스터디, 비교 사례분석 등)과 주요 결과 요약.

모델별 역할을 감안해,
- GPT는 어떤 AV 회사/프로젝트가 좋은 예시인지 후보를 넓게 제시하고,
- Claude는 케이스 선택 논리와 empirics 스토리를 정리하며,
- Claude Code는 데이터 수집/코딩 스킴/분석 스텝을 pseudo-code와 표로 설계하고,
- Gemini는 케이스 선택 바이어스, 식별문제, 대안 설명, robustness check 아이디어를 비판적으로 제안한다.
