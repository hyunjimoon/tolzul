...
1. **Environment**: Each industry environment is characterized by distinct ground truth probabilities of success:
...
The simulation results directly connect to the empirical patterns documented in Tables 2 and 3, providing a theoretical foundation for observed industry-specific investment behaviors.
...
![[Pasted image 20241103201944.png]]
...
| **Success Factors**    | - Fast market feedback loops<br>- Flexible operations<br>- Strong partner network                                                                                                                                                                                                                                                                         | - Strong capital base<br>- Clear architectural vision<br>- Integrated control systems                                                                                                                                                                                                                                                     | - Operational excellence<br>- Efficient processes<br>- Cost leadership                                                                                                           |
...
data {
  int<lower=0> N;  
  array[N] int<lower=0, upper=1> action;   // Actions taken
  array[N] int<lower=0, upper=1> revenue;  // Revenue signals
  array[N] int<lower=0, upper=1> cost;     // Cost signals
  real<lower=0> sigma; 
}

parameters {
  real p_c;  // ROI of operational investment
  real p_r;  // ROI of market investment
}

model {
  // Priors for p_c and p_r, centered around 1.0 to reflect initial belief
  p_c ~ beta(1.0, 1.0);
  p_r ~ beta(1.0, 1.0);
