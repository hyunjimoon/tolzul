...
idm_param = {'T': 1, 'a': 2, 'b': 1.5, 's_min': 1, 'v_max': 25, 'sigma': 4}
...
class Car:
    ...
    def IDM_model(self, dt, vehicle_lead, reference_position_x):
        ...
        v = self.velocity
        delta_v = self.velocity - vehicle_lead.velocity
        s_star = idm_param['s_min'] + max(0, v.x * idm_param['T'] 
                + v.x * delta_v.x / (2 * math.sqrt(idm_param['a'] * idm_param['b'])))
        a = idm_param['a'] * (1 - pow((self.velocity.x / idm_param['v_max']), idm_param['sigma'])
                - pow((s_star / current_gap.x), 2))

        # TODO: You need to set the vehicle's new acceleration here by setting 'self.acceleration' parameter
        self.prev_acc.x = self.acceleration.x
    ...
    def next_step(self, dt, reference_position_x):
        ...
        # print(f'type of prev_acc is {type(self.prev_acc)}')
        velocity_change.xy = self.prev_acc.x * dt, 0
    ...
    def Custom_model(self, dt, vehicle_lead, vehicle_follow, reference_position_x):
        ...
        s_star = lambda car, front: idm_param['s_min'] + max(0, car.velocity.x * idm_param['T'] 
                + car.velocity.x * (car.velocity.x - front.velocity.x) /\
                    (2 * math.sqrt(idm_param['a'] * idm_param['b'])))

        desired_acc = pow((self.velocity.x / idm_param['v_max']), idm_param['sigma'])

        braking_lead = s_star(self, vehicle_lead) / current_lead_gap.x
        braking_follow = min(1, (s_star(vehicle_follow, self) / current_follow_gap.x))

        if self.acceleration.x >= 0:
            braking = pow(braking_lead, 2)
        else:
            braking = pow(braking_lead - braking_follow, 2)

        a = idm_param['a'] * (1 - desired_acc - braking)
