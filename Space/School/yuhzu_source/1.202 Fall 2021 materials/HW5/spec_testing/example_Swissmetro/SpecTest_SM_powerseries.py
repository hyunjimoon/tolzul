...
B_TRAIN_TIME_SQ	 = Beta('B_TRAIN_TIME_SQ',0,None,None,0)
...
TRAIN_TT_SQ  = DefineVariable('TRAIN_TT_SQ', TRAIN_TT_SCALED   *  TRAIN_TT_SCALED,database  )
...
M2_SBB_SP = ASC_SBB + B_TRAIN_TIME * TRAIN_TT_SCALED + B_TRAIN_TIME_SQ * TRAIN_TT_SQ + B_TRAIN_COST * TRAIN_COST_SCALED + B_HE * TRAIN_HE_SCALED + B_GA * GA
...
def utilPower(tt):
    tt_scaled = tt / 100
    betas = ['B_TRAIN_TIME','B_TRAIN_TIME_SQ']
    betaValues = results_M2.getBetaValues(betas)
    V = betaValues['B_TRAIN_TIME'] * tt_scaled + betaValues['B_TRAIN_TIME_SQ'] * tt_scaled * tt_scaled 
    return V
