...
Total_TT1_cu = Beta('Total_TT1_cu',0,None,None,0)
...
TripTimeHours_1_sq  = DefineVariable('TripTimeHours_1_sq', TripTimeHours_1 * TripTimeHours_1  ,database)
TripTimeHours_1_cu  = DefineVariable('TripTimeHours_1_cu', TripTimeHours_1* TripTimeHours_1 * TripTimeHours_1 ,database)
...
M2_Opt1 = Constant1 + Fare * Fare_1 + Legroom * Legroom_1 + SchedDE * Opt1_SchedDelayEarly + SchedDL * Opt1_SchedDelayLate + Total_TT1 * TripTimeHours_1 + Total_TT1_sq * TripTimeHours_1_sq  + Total_TT1_cu * TripTimeHours_1_cu
...
def utilPower(tt):
    betas = ['Total_TT1','Total_TT1_sq','Total_TT1_cu']
    betaValues = results_M2.getBetaValues(betas)
    V = betaValues['Total_TT1'] * tt + betaValues['Total_TT1_sq'] * tt * tt  + betaValues['Total_TT1_cu'] * tt * tt * tt
    return V

travelTime = np.arange(0,6.5,0.05)
vl = [utilLinear(t) for t in travelTime]
vpw = [utilPower(t) for t in travelTime]

fig, ax = plt.subplots()
ax.plot(travelTime,vl,label='Linear')
ax.plot(travelTime,vpw,label='Power series')
ax.legend()
