...
cost111  = DefineVariable('cost111', cost1 * cost1 * cost1 ,database)
cost222  = DefineVariable('cost222', cost2 * cost2 * cost2 ,database)
...
#Utilities power series
M2_V_BM = ASC_BM + B_MCOST1 * cost1 + B_MCOST2 * cost11 / 1000 + B_MCOST3 * cost111 / 1000000
M2_V_SM = ASC_SM + B_MCOST1 * cost2 + B_MCOST2 * cost22 / 1000 + B_MCOST3 * cost222 / 1000000
...
def utilPower(cost):
    ...
    V = betaValues['B_MCOST1'] * cost + betaValues['B_MCOST2'] * cost * cost / 1000 + betaValues['B_MCOST3'] * cost * cost * cost / 1000000
    return V

costs = np.arange(0,450,0.5)
vl = [utilLinear(c) for c in costs]
vpw = [utilPower(c) for c in costs]

fig, ax = plt.subplots()
ax.plot(costs,vl,label='Linear')
ax.plot(costs,vpw,label='Power series')
ax.legend()
plt.xlabel("Cost")
plt.ylabel("Utility")
plt.show()
