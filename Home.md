---
created: 2025-10-16
type: map
rank: 5
modified:
  - 2025-10-17T14:50:14-04:00
  - 2025-11-01T22:25:18-04:00
  - 2025-11-02T00:40:23-04:00
  - 2025-11-07T22:36:43-05:00
  - 2025-11-09T04:50:31-05:00
---

> *"뉴런이 함께 발화하면 함께 묶인다"*  
> **Neurons that fire together, wire together**

---

## 🎯 Bayesian Daily Loop

> 듣는 귀(RO)와 보는 눈(VOI)에서 들어온 목적함수를,  
> 조율하는 손(Balance)이 최적화한다.  
> [[Space|👂 Hear]] → [[Front|👁 See]] → [[Balance|✋ Tune]]

| 감각                             | 역할                         | 작용                       | 변수  |
| ------------------------------ | -------------------------- | ------------------------ | --- |
| 👂 **[[Space\|Space (귀)]]**    | RO (Real Option)           | 여백을 듣는다. 불확실성을 보존한다.     | 확장  |
| 👁 **[[Front\|Front (눈)]]**    | VOI (Value of Information) | 명료하게 본다. 실행으로 수렴한다.      | 수렴  |
| ✋ **[[Balance\|Balance (손)]]** | Control                    | 리듬을 조율한다. 여백의 폭 V를 조절한다. | 안정화 |

---

## 🌊 Day Flow

- **아침:** 👂 [[Space|Space (귀)]] — 가능성을 듣기 → 여백 확장 *(RO)*  
- **낮:** 👁 [[Front|Front (눈)]] — 결심을 보기 → 명료화 *(VOI)*  
- **저녁:** ✋ [[Balance|Balance (손)]] — 리듬을 조율하기 → 여백 안정화 *(Control)*

---
[[전라좌수군 견리사의 군령]]

## 🧭 Navigation

> [!globe] **👂 Space RO**
> [[Space/Library/Maps/Maps|Maps]] | [[Space/Sources/Papers|Papers]] | [[Space/Dots|Dots]]

> [!calendar] **✋ Balance **
> [[Balance/일별 리듬|Daily]] | [[Balance/주별 멜로디|Weekly]] | [[Balance/월별 화음|Monthly]]

> [!mountain] **👁 Front 눈 VOI**
> [[On|On]] | [[Ongoing|Ongoing]] | [[Simmering|Simmering]]


---

![[귀손눈.png|500]]

> [[Space|Space]]와 [[Front|Front]]는 서로 반대방향의 힘이다.  
> [[Balance|Balance]]는 그 사이에서 여백 (V)의 진폭을 제어한다.  
> 하루는 곧 하나의 Bayesian 최적화 루프이다.
