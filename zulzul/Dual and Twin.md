...
Dual is a relation where the pair defines each other while Twin is [[🌅⏱ Weak Copy]]. Example of dual is [[def(Generator)]] and [[def(Discriminator)]] while example of twin is two solutions with the same distribution but with different construction. Might dual be related to twin? Yes. 
